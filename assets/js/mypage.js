...
async function MypageShow() {
  ...
  user_created_at.innerText =
    "가입일 : " + response_json.user.created_at.substr(0, 10);
  ...
  bid_json.forEach((bid) => {
    const id = bid.article.id;
    const product = bid.article.product;
    const max_point = bid.max_point;
    const image_url = bid.article.image;
    const image = `${BACKEND_API}${image_url}`;
